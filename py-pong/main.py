...
def run():
    ...
    # TODO, read configuration from file. pickle would be easiet to do that, though
    # that means writing a generation script.
    configuration = {
        'screen_size': (5800,2430),#(1920, 1200),
    ...
    # num_of_clients = int(ips.readline().strip())
    ips.readline() #comment line
    address = ips.readline().strip()
    print address
    while address:
        clisocket.append(socket.socket(socket.AF_INET, socket.SOCK_STREAM))
        clisocket[-1].connect((address, 20000)) # get the last element in list
        address = ips.readline().strip()
        print address
